...
*   **How SmartServ uses it**: 
    1.  [UserController](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/controller/UserController.java) (Controller Layer) receives HTTP requests.
    2.  [InvoiceServiceImpl](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/service/InvoiceServiceImpl.java) (Service Layer) handles calculations and business rules.
    3.  [JobCardRepository](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/repository/JobCardRepository.java) (Repository Layer) fetches data from the database.
...
*   **How SmartServ uses it**: We use Lombok's `@Builder` annotation on DTOs like [InvoiceResponseDto](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/dto/invoice/InvoiceResponseDto.java). It allows us to write:
    ```java
...
*   **How SmartServ uses it**: We use Lombok's `@RequiredArgsConstructor` to generate a constructor for our `final` helper fields. Look at [InvoiceServiceImpl](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/service/InvoiceServiceImpl.java#L46-L53):
    ```java
    @Service
...
*   **How SmartServ uses it**: In [InvoiceServiceImpl](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/service/InvoiceServiceImpl.java#L45-L49), we have `@Transactional` at class level.
...
*   **How SmartServ uses it**:
    *   **Backend**: [JwtAuthFilter](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/security/JwtAuthFilter.java) intercepts every request. It extracts the JWT cookie/token, checks if it is valid, and logs the user details into Spring Security.
    *   **Frontend**: [axiosConfig.js](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/SmartServFrontEnd/src/api/axiosConfig.js#L23-L39) uses response interceptors to watch for `401 Unauthorized` errors and automatically logout the user.
...
*   **How SmartServ uses it**:
    ...
    *   On the frontend, in [invoiceService.js](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/SmartServFrontEnd/src/services/invoiceService.js#L3-L16), `normalizeInvoice` adapts raw JSON data:
        ```javascript
...
*   **How SmartServ solves it**:
    1.  **Fetch Joins**: Inside [JobCardRepository](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/repository/JobCardRepository.java#L18-L25), we write custom `LEFT JOIN FETCH` queries to load parents and children in a single database roundtrip.
    2.  **Entity Graphs**: In [JobCard.java](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/entity/JobCard.java#L28-L50), we define `@NamedEntityGraph` listing what relationships must be fetched together.
...
*   **How SmartServ uses it**:
    1.  If the Axios interceptor in [axiosConfig.js](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/SmartServFrontEnd/src/api/axiosConfig.js#L31) detects a `401 Unauthorized` error, it publishes an event:
        ```javascript
        window.dispatchEvent(new Event('auth:unauthorized'));
        ```
    2.  The [AuthContext.jsx](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/SmartServFrontEnd/src/context/AuthContext.jsx#L47-L52) component subscribes to this event:
        ```javascript
...
*   **How SmartServ uses it**: We use [AuthContext.jsx](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/SmartServFrontEnd/src/context/AuthContext.jsx#L108-L113) to wrap our application components:
    ```javascript
    return (
...
*   **How SmartServ uses it**: Spring Security's password encoding relies on the `PasswordEncoder` strategy interface. In [SecurityConfig.java](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/src/main/java/com/smartserv/config/SecurityConfig.java#L53-L56), we choose the `BCryptPasswordEncoder` strategy:
    ```java
    @Bean
...
*   [SmartServ Design Patterns & Architecture Reference](file:///c:/Shivam%20New/PROJECT%202026/SmartServ/docs/DESIGN_PATTERNS.md)
